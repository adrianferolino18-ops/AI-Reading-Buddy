...
@section('title', 'Assessment Selection Hub')
...
<div class="max-w-5xl mx-auto px-4 py-10">
    ...
    @if($modules->isEmpty())
        <div class="flex flex-col items-center justify-center text-center py-12 max-w-md mx-auto bg-slate-900 border border-slate-800 rounded-2xl p-8 mt-10 shadow-xl">
            ...
            <a href="{{ route('library') }}" class="mt-6 inline-flex items-center justify-center bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs px-5 py-2.5 rounded-lg transition-all shadow-[0_0_15px_rgba(99,102,241,0.3)]">
            ...
            @foreach($modules as $module)
                <div class="group rounded-3xl border border-white/10 bg-slate-900 p-6 shadow-xl shadow-black/10 transition hover:-translate-y-1 hover:border-indigo-500/30">
                    <div class="mb-4 flex items-center justify-between gap-3">
                        ...
                        <span class="text-xs text-slate-500">ID {{ $module->id }}</span>
                    </div>
                    <h2 class="text-xl font-bold text-white">{{ $module->title }}</h2>
                    <p class="mt-3 text-sm leading-6 text-slate-400">{{ \Illuminate\Support\Str::limit($module->body_text ?? 'No description available.', 110) }}</p>
                    <div class="mt-8">
                        <a href="{{ route('quiz.index', ['module_id' => $module->id]) }}" class="inline-flex w-full items-center justify-center rounded-2xl bg-gradient-to-r from-purple-600 to-blue-600 px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-purple-500/20 hover:from-purple-500 hover:to-blue-500">Enter Quiz Arena</a>
                    </div>
                </div>
            @endforeach
        </div>
